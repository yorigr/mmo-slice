// MonsterManager — spawn, IA, loot de monstros
const { v4: uuidv4 } = require('uuid');
const {
  MONSTER_SPAWN_INTERVAL_MS, MONSTER_AI_TICK_MS, MONSTER_AGGRO_RANGE,
  MONSTER_LEASH_RANGE, MAP_W, MAP_H,
} = require('../config/constants');

// Definições de monstros (serão movidas para monsters.json futuramente)
const MONSTER_TYPES = {
  goblin: {
    name: 'Goblin',
    hp: 40, maxHp: 40, speed: 120,
  ...
  orc: {
    name: 'Orc',
    hp: 80, maxHp: 80, speed: 90,
    damage: 18, range: 65, attackCooldown: 2000,
    xpReward: 70, goldReward: [15, 40],
    lootTable: [
      { itemId: 'axe_iron', chance: 0.2 },
      { itemId: 'armor_leather', chance: 0.1 },
      { itemId: 'potion_small', chance: 0.4 },
    ],
  },
  skeleton: {
    name: 'Skeleton',
    hp: 55, maxHp: 55, speed: 110,
...
class MonsterManager {
  ...
  // Spawna um monstro aleatório em posição aleatória
  spawnRandom() {
  ...
  aiTick(now) {
    ...
    for (const m of this.world.monsters.values()) {
      if (m.hp <= 0) { this.world.removeMonster(m.id); continue; }

      // Encontrar alvo mais próximo (se não tem um)
      ...
      if (m.state === 'aggro' && m.target) {
        ...
        if (!target || target.dead) {
          ...
          // Leash — voltou longe demais do spawn
      ...
      if (m.state === 'returning') {
        ...
        if (d < 5) {
          ...
          m.hp = m.maxHp; // full regen ao voltar ao spawn
        } else {
  ...
  onMonsterDeath(monster, killerId) {
    // XP e gold para o killer
    if (killerId) {
      const killer = this.world.getPlayer(killerId);
      if (killer) {
        killer.xp   += monster.xpReward;
        const gold   = monster.goldReward[0] + Math.floor(Math.random() * (monster.goldReward[1] - monster.goldReward[0]));
        killer.gold += gold;
        this.world.io.to(killerId).emit('player:xp', { xp: monster.xpReward, gold });
      }
    }
    ...
    for (const entry of monster.lootTable) {
      if (Math.random() < entry.chance) {
        const item = { id: uuidv4(), type: entry.itemId, x: monster.x + (Math.random()-0.5)*30, y: monster.y + (Math.random()-0.5)*30 };
        this.world.addItem(item);
        setTimeout(() => this.world.removeItem(item.id), require('../config/constants').LOOT_DESPAWN_MS);
      }
    }
